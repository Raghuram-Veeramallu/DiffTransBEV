...
class TransformerPositionalEmbedding(nn.Module):
    ...
    """
    def __init__(self, dimension, max_timesteps=1000, clip_value=None, scale_factor=0.01):
        ...
        div_term = torch.exp(even_indices * -log_term) * scale_factor
        ...
        # Normalize the positional encoding matrix
        self.pe_matrix = self.pe_matrix / self.pe_matrix.abs().max()

        # Clipping values (optional)
        self.clip_value = clip_value
        if self.clip_value is not None:
            self.pe_matrix = torch.clamp(self.pe_matrix, -self.clip_value, self.clip_value)

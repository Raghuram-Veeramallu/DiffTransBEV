...
from data_loader.nuscenes import NuScenesDataLoader
from losses.simple_loss import SimpleLoss
from models.DiffDiTBEVModel import DiffDiTBEV
from utils import distributed_utils, eval_utils, logging_utils, param_utils
from utils.data_prefetcher import data_prefetcher


class DiffDiTBEVTrainer:
    ...
    def __init__(self, config) -> None:
        ...
        """Initiate the logger"""
        self.logging = logging_utils.get_std_logging(config.logging.logfile, logging_utils.logging_levels[config.logging.level])

        """device parameters"""
        self.world_size = self.config.world_size
        self.rank = self.config.rank
        self.gpu = self.config.local_rank
        self.distributed = self.config.distributed_run
        ...
        self.ckpt_path = self.config.checkpoint.ckpt_path.format(
            self.time_stamp,self.time_stamp, self.config.model.backbone_type, {})

        save_dir = '/'.join(self.ckpt_path.split('/')[:-1])
        self.log_all = self.config.logging.log_all
        
        """Wandb Log"""
        self.enable_wandb = self.config.logging.enable_wandb
        if self.enable_wandb:
            import wandb
            if self.gpu==0 or self.log_all:
                wandb.init(project="diff-det-bev",name = save_dir+'_gpu_'+str(self.rank))
        ...
        self.steps = 0
        self.epoch_count = -1
        self.log_step = self.config.logging.log_step
        self.logging = logging_utils.get_std_logging(self.config.logging.logfile, logging_utils.logging_levels[config.logging.level])
        if self.rank == 0:
            self.writer = SummaryWriter(self.config.logging.logdir)
    
    def construct_model(self):
        """get data loader"""
        self.stage = self.config.stage
        assert self.stage == 'train', ValueError(f'Invalid stage: {self.stage}, only "train" for training')
        self.data_ins = NuScenesDataLoader(self.config)
        self.train_loader = self.data_ins.get_dataloader(self.config, self.stage, self.train_batch_size)

        self.sync_bn = self.config.amp.sync_bn
        self.opt_level = self.config.amp.opt_level
        self.logging.info(f"sync_bn: {self.sync_bn}")

        """build model"""
        self.logging.info('Initializing the DiffDiTBEV Model')
        net = DiffDiTBEV(self.config, self.device)
        if self.sync_bn:
            net = torch.nn.SyncBatchNorm.convert_sync_batchnorm(net)
        self.model = net.to(self.device)
        self.logging.info('DONE Initializing DiffDiTBEV Model')

        """build optimizer"""
        weight_decay = self.config.optimizer.weight_decay
        exclude_bias_and_bn = self.config.optimizer.exclude_bias_and_bn
        params = param_utils.collect_params([self.model],
                                            exclude_bias_and_bn=exclude_bias_and_bn)
        self.optimizer = torch.optim.AdamW(params, lr=self.max_lr, weight_decay=weight_decay)
    ...
    def train_epoch(self, epoch, printer=print):
        batch_time = eval_utils.AverageMeter()
        data_time = eval_utils.AverageMeter()
        forward_time = eval_utils.AverageMeter()
        backward_time = eval_utils.AverageMeter()
        log_time = eval_utils.AverageMeter()
        loss_meter = eval_utils.AverageMeter()
        ...
        images, rots, trans, intrins, anns = prefetcher.next()
        i = 0
        while all(var is not None for var in [images, rots, trans, intrins]):
            i += 1
            ...
            with torch.cuda.amp.autocast():
                ...
                # forward
                tflag = time.time()
                res_images = images.view(-1, images.size(2), images.size(3), images.size(4))
                outputs = self.model(res_images, rots, trans, intrins, anns)
                forward_time.update(time.time() - tflag)

                tflag = time.time()
                loss = self.forward_loss(anns, outputs)
